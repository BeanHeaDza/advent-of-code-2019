...
const guesser = async () => {
  let knownPaths = [];
  let lastPath;
  while (Array.isArray(knownPaths)) {
    lastPath = knownPaths;
    knownPaths = await run(knownPaths);
  }
  fs.writeFileSync("q13-path.json", lastPath, { encoding: "utf8" });
  ...
  readline.question(
    `At what speed do you want to see the ping pong simulate?`,
    speed => {
      run(lastPath, +speed).then(() => {});
      readline.close();
    }
  );
